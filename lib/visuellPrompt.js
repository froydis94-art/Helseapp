 ...
 * Scale: ~3mo noticeable/moderate → ~6mo clearly stronger → ≥12mo most dramatic.
...
function mainGoalLines(mode, months, bfGoal, bfNow) {
  ...
  if (intensity === "noticeable") {
    return [
      ...
      "Some clearer shoulder/arm outline — moderate recomposition, not toning-only.",
      deltaCue.trim(),
    ].join(" ");
  }
  if (intensity === "strong") {
 ...
 * @returns {{ prompt: string, months: number, mode: string, zones: string[], intensity: string }}
 */
function byggVisuellPrompt({
  ...
  return { prompt, months: m, mode, zones: zoneList, intensity };
}
