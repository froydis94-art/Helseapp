...
function getFatPhrase({ fat, bfNow, bfGoal, level }) {
  ...
  const table = [
    ...
    "significant fat loss: the waist is clearly narrower and tighter than the source, subcutaneous fat over the lower abdomen and chest is visibly reduced to match a lean athletic physique",
    veryLeanTarget
      ? "dramatic fat loss to a very lean athletic look with clear natural ab/oblique definition"
...
function buildModelInput(
  ...
  if (name.includes("flux-kontext") || /^black-forest-labs\//i.test(name)) {
    ...
    // Upsampling helps large BF deltas but often worsens E005 on shirtless fat-loss
    // (we can't detect clothing from the image). Disable when strong intensity + fat
    // decrease — the common bare-torso fitness case. Keep upsampling for large BF
    // deltas on milder intensity (more often clothed) or fat-increase edits.
    const highShirtlessRisk =
      extras.fat === "decrease" && intensity === "strong";
    const useUpsampling = needStrongEdit && !highShirtlessRisk;
