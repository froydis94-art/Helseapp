...
const HORIZON_COPY = {
  ...
  "12w": {
    ...
    months: 3,
    change:
      "short-horizon progress: visibly narrower waist, clear midsection fat reduction, early athletic outline — noticeable but not a total makeover",
  },
  "24w": {
  ...
  "18m": {
    ...
    months: 18,
    change:
      "total long-term transformation: significant body recomposition, clearly narrower waist, major soft-tissue loss, clear athletic muscle outline — still a real photograph of this person, not a caricature",
  },
};
...
function buildPromptHolistic(maal, intensity = "moderate", extras = {}) {
  ...
  const timelineForce =
    months >= 12
      ? `TIMELINE MULTIPLIER (~${months} months, distance ${distance?.score ?? "?"}/10): rich total-body silhouette change matching this long horizon — reshape waist, soft-tissue pads, and muscle volume with clear visual delta.`
      : months <= 3
        ? `TIMELINE MULTIPLIER (~${months} months, distance ${distance?.score ?? "?"}/10): noticeable moderate recomposition — narrower waist, reduced midsection soft tissue, early muscle outline — still visibly different from the source.`
