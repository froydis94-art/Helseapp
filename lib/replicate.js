...
const HORIZON_COPY = {
  "4w": {
    ...
    change:
      "show early but unmistakable progress — tighter midsection, fresher posture, light muscle tone that is obvious side-by-side with the original",
  },
  "8w": {
    label: "about 8 weeks from now",
    change:
      "show clear mid-plan transformation — leaner waist, more athletic arms and shoulders, visibly fitter body composition",
  },
  "12w": {
    label: "about 3 months from now",
    change:
      "show strong plan-completion results — much leaner midsection, clear muscle definition, athletic physique that looks earned",
  },
  "24w": {
    label: "about 6 months from now",
    change:
      "show major fitness transformation — significantly leaner and more muscular athletic body while still looking like the same person",
  },
  "18m": {
    label: "about 1.5 years from now",
    change:
      "show peak long-term goal physique — dramatic but photorealistic athletic transformation with lean definition and confident posture",
  },
};

const FOCUS_COPY = {
  overall:
    "full-body athletic upgrade: leaner silhouette, better muscle tone, tighter core, and stronger posture",
  cardio:
    "endurance-athlete look: leaner frame, lighter midsection, healthier vascular conditioning cues, energetic posture",
  core:
    "core-first transformation: clearly flatter tighter waistline, stronger abs/obliques appearance, firmer midsection",
  strength:
    "strength-focused build: thicker athletic muscle in arms, shoulders, chest and legs with realistic definition",
  posture:
    "posture-led upgrade: taller stance, open shoulders, lifted chest, plus a visibly fitter leaner body supporting that presence",
};
 ...
 * Body composition MUST change visibly; face/clothes/pose stay locked.
 */
function buildPrompt(maal, intensity = "moderate", extras = {}) {
  ...
  const strength =
    intensity === "strong"
      ? "a dramatic, obvious athletic body transformation that is still photorealistic"
      : intensity === "subtle"
        ? "a clearly noticeable early fitness upgrade (not a tiny touch-up)"
        : "an obvious mid-level athletic body transformation that is easy to spot";

  return [
    "Edit this exact person into a motivational before/after fitness visualization of their future self.",
    `Timeline: ${horizonInfo.label}. ${horizonInfo.change}.`,
    "CRITICAL: the body change must be unmistakable when compared with the original photo. Do not output a near-identical image.",
    "Change body composition and physique shape to match the timeline and focus. Make waist, arms, shoulders, chest, and core look clearly fitter.",
    "Keep the identical face, hair, facial hair, skin tone, age cues, pose, camera angle, framing, and background.",
    ...
    `Apply ${strength}.`,
    "Photorealistic smartphone photo quality, natural lighting, sharp detail, no text, no watermark, no logo.",
  ].join(" ");
}
...
function buildImg2ImgPrompt(maal, intensity = "moderate", extras = {}) {
  ...
  return [
    `Photorealistic smartphone photo of the exact same person ${horizonInfo.label}, with an ${amount} athletic body transformation.`,
    `Personalized goal brief: ${brief}.`,
    "Body composition must look clearly fitter than the source: leaner waist, stronger muscle tone, better posture cues.",
    ...
    "Realistic skin, natural lighting, no stylization, no near-identical copy of the source physique.",
  ].join(" ");
}
...
function buildModelInput(
  ...
  return {
    ...
    image: imageDataUri,
    prompt_strength:
      intensity === "strong" ? 0.55 : intensity === "subtle" ? 0.38 : 0.46,
