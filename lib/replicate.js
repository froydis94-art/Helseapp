...
function composeGoalBrief({
  ...
  const resolvedOutcomes = normalizeOutcomes(outcomes);
  ...
  const muscleInfo = MUSCLE_COPY[resolvedMuscle] || MUSCLE_COPY.toned;
  const frameInfo = FRAME_COPY[frame] || FRAME_COPY.average;
  const shapeInfo = SHAPE_COPY[shape] || "";
  ...
  if (outcomeInfo) parts.push(`desired outcomes: ${outcomeInfo}`);
  ...
  if (Number.isFinite(bfA) && Number.isFinite(bfB) && bfA > 0 && bfB > 0) {
    parts.push(
      `body-fat change from about ${bfA}% toward about ${bfB}% (motivational estimate only)`
    );
  }
...
function buildPrompt(maal, intensity = "moderate", extras = {}) {
  const { brief, horizonInfo, fatInfo, muscleInfo, zoneInfo, shapeInfo, outcomeInfo, frameInfo } =
    composeGoalBrief({
      maal,
      ...extras,
    });
  ...
  return [
    ...
    shapeInfo
      ...
      : "",
    outcomeInfo ? `Motivation outcomes: ${outcomeInfo}.` : "",
    force,
...
function buildImg2ImgPrompt(maal, intensity = "moderate", extras = {}) {
  const { brief, horizonInfo, fatInfo, muscleInfo, zoneInfo, shapeInfo, outcomeInfo } =
    composeGoalBrief({
      maal,
      ...extras,
    });
