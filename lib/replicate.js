...
// Used when Flux safety filter (E005) blocks shirtless/physique progress photos.
const DEFAULT_FALLBACK_MODEL = "stability-ai/stable-diffusion-img2img";
...
function buildModelInput(model, { imageDataUri, maal, intensity }) {
  ...
  // SD / SDXL-style img2img fallback (more tolerant of physique progress photos)
  return {
    prompt: buildImg2ImgPrompt(maal, intensity),
    negative_prompt: buildNegativePrompt(),
    image: imageDataUri,
    prompt_strength:
      intensity === "strong" ? 0.55 : intensity === "subtle" ? 0.35 : 0.45,
...
async function generateWithReplicate({
  ...
  try {
    ...
    const shouldFallback =
      fallbackModel &&
      fallbackModel !== model &&
      isSafetyBlock(primaryError.message);
