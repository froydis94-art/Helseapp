...
function horizonChangeForFat(horizonInfo, fat, bfNow, bfGoal) {
  ...
  if (abs >= 4) {
    ...
    return `${core}. Horizon realism (${label}): the fat-loss delta must look earned and photorealistic for that timeframe — motivating, not overnight magic or CGI`;
  }
  return `${horizonInfo.change} — keep progress believable for ${label}`;
}
...
function getFatPhrase({ fat, bfNow, bfGoal, level }) {
  ...
  const table = [
    "visible fat loss: narrower waist and less soft tissue on the lower abdomen",
    "clear fat loss: noticeably narrower waist circumference, reduced belly fat layer, leaner chest",
    "significant fat loss: the waist is clearly narrower and tighter than the source, subcutaneous fat over the lower abdomen and chest is visibly stripped down to match a lean athletic physique",
    veryLeanTarget
      ? "dramatic fat loss to a very lean athletic look with sharp natural ab/oblique definition"
      : "major fat loss: waist circumference clearly reduced, belly soft tissue visibly gone compared to source, lean athletic midsection matching the target %, still photorealistic",
  ];
...
function challengeDetailLine(zones, fat, level) {
  ...
  if (focusAbs) {
    bits.push(
      level >= 2
        ? "love handles and lower-belly soft tissue are sharply reduced; the waist looks pinched, narrower and tightly cinched"
  ...
  if (list.includes("arms")) {
    bits.push(
      level >= 2
        ? "arms look leaner with commanding muscle separation and less soft cover"
  ...
  if (list.includes("shoulders") || list.includes("chest") || list.includes("back")) {
    bits.push(
      level >= 2
        ? "shoulders and upper torso look more athletic with sharper separation"
  ...
  if (list.includes("glutes")) {
    bits.push(
      level >= 2
        ? "glutes look tighter and more athletic in proportion to the leaner waist"
 ...
 */
function buildForcedChangeBlock({ fat, bfNow, bfGoal, level, zones }) {
  const now = Number(bfNow);
  const goal = Number(bfGoal);
  const abs = Math.abs(fatDeltaPoints(bfNow, bfGoal));
  ...
  const hard =
    level >= 2
      ? [
          "HOWEVER, the body composition has changed significantly: waist is noticeably narrower and tighter, subcutaneous fat over the lower abdomen and torso is visibly reduced, matching a lean " +
            `${goal}% body fat physique (was about ${now}%).`,
          "Abdominal muscles and obliques are sharply defined. Shoulder and arm muscle separation is visibly enhanced.",
      ...
      : [
          "HOWEVER, change the body composition clearly from the source:",
          "Narrow the waist and reduce the soft fat layer on the lower abdomen and love handles.",
          `Aim for a leaner about ${goal}% body fat look versus about ${now}% in the source.`,
          "Make arm and shoulder definition clearer.",
          challengeDetailLine(zones, "decrease", level),
...
function getMusclePhrase({ fat, muscleKey, outcomes, level, bfGoal }) {
  ...
  if (wantsVolume) {
    const table = [
      ...
      "noticeably fuller athletic muscle — wider shoulders, more arm/chest volume, sharper separation",
      "stronger athletic muscle volume with commanding wider shoulders and deep but realistic definition",
    ];
...
function buildHolisticNarrative({
  ...
  const story = [
    ...
    buildForcedChangeBlock({ fat, bfNow, bfGoal, level, zones }),
    identity + ".",
...
function buildPrompt(maal, intensity = "moderate", extras = {}) {
  ...
  // Order: professional base → change language (in story) → soft identity → realism.
  // Flux Kontext Pro has NO image_strength — intensity is language + prompt_upsampling.
  return [
    "A high-end professional fitness photo, exact same person, face, and room as the original image.",
    "PRIMARY TASK — body recomposition (must be obvious vs the source):",
    story,
    force,
    ...
    "CRITICAL STYLE: real smartphone / fitness photo — natural skin, real lighting, real proportions. No cartoon/CGI/anime.",
...
function buildModelInput(
  ...
  if (name.includes("flux-kontext") || /^black-forest-labs\//i.test(name)) {
    // Flux Kontext Pro has NO image_strength — only prompt drives change.
    // For meaningful BF drops, allow light prompt upsampling to reinforce edit intent.
    const now = Number(extras.bfNow);
    const goal = Number(extras.bfGoal);
    const abs =
      Number.isFinite(now) && Number.isFinite(goal) ? Math.abs(now - goal) : 0;
    const needStrongEdit =
      (extras.fat === "decrease" || extras.fat === "increase") && abs >= 4;
    ...
    return {
      ...
      output_format: "png",
      safety_tolerance: 5,
      prompt_upsampling: needStrongEdit,
    };
  }
...
async function generateWithReplicate({
  ...
  console.info(
    `[replicate] Route: ${primaryLabel} first (${preferMax ? "demanding edit" : "mild edit"})`
  );
