...
const HORIZON_COPY = {
  ...
  "24w": {
    ...
    change:
      "strong but believable recomposition: lower body fat and better muscle definition while staying photorealistic",
  },
  "18m": {
    label: "about 1.5 years from now",
    change:
      "mature long-term athletic progress: leaner and more developed physique that still looks like a real photograph of this person — not a comic or bodybuilder caricature",
  },
};
...
function buildPrompt(maal, intensity = "moderate", extras = {}) {
  ...
  const force =
    intensity === "strong"
      ? "Apply a clearly visible but still realistic fitness change matching the selected parameters."
      : intensity === "subtle"
        ? "Apply a modest realistic fitness upgrade matching the selected parameters."
        : "Apply an obvious but natural fitness upgrade matching the selected parameters.";
