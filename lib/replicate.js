...
// Per-attempt budget so Max→Pro→Dev can fall through inside Vercel maxDuration (180s).
// Old POLL_TIMEOUT_MS=150s was per attempt and burned the whole request on one dead model.
const ATTEMPT_POLL_TIMEOUT_MS = 50000;
// Cap Replicate Prefer: wait so create doesn't block ~60s before we can poll/fail-fast.
const CREATE_WAIT_SECONDS = 20;
...
function bodyFatDeltaVisual(bfNow, bfGoal, fat) {
  ...
  const waistLine = bigDrop
    ? "Clear athletic fat loss around the waist and lower abdomen, significantly tighter midsection, natural ab and oblique definition, reduced subcutaneous fat across the chest and torso"
    : midDrop
      ? "clear fat loss around the waist and lower abdomen, tighter midsection, firmer natural ab outline, reduced soft tissue on the torso"
...
function leggTilEkstremeVisuelleMarkører(months, fat, bfGoal) {
  ...
  if (fat === "increase") {
    return `The body composition has changed profoundly over the ${years} timeline: clearly fuller softer silhouette with major soft-tissue gain matching about ${Number(bfGoal) || "the target"}% body fat — not a near-copy of the source.`;
  }
  if (fat === "maintain") {
    return `Over the ${years} timeline muscle shape is clearly more developed while body-fat softness stays similar — visible long-term training progress, not a near-copy.`;
  }
  return [
    `The body composition has changed profoundly over the ${years} timeline.`,
    "The waistline is markedly narrower and tighter.",
    "Subcutaneous fat over the lower abdomen, love handles, and torso is largely eliminated, matching a lean athletic physique.",
    "Abdominal muscles and obliques are sharply sculpted and visible.",
...
function buildForcedChangeBlock({ fat, bfNow, bfGoal, level, zones, months }) {
  ...
  const hard =
    level >= 2
      ? [
          "HOWEVER, athletic body composition has changed significantly: waist is noticeably narrower and tighter, subcutaneous fat over the lower abdomen and torso is visibly reduced, matching a lean " +
            `${goal}% body fat fitness physique (was about ${now}%).`,
          "Natural ab and oblique definition is clearly visible. Shoulder and arm muscle separation is enhanced in a realistic athletic way.",
      ...
      : [
          ...
          "Slightly tighter waist, subtle fat reduction and toning on the lower abdomen and love handles.",
...
function buildHolisticNarrative({
  ...
  return { story, level, fatPhrase, musclePhrase, months: m };
}
...
function buildPrompt(maal, intensity = "moderate", extras = {}) {
  ...
  const timelineForce =
    months >= 12
      ? `TIMELINE MULTIPLIER (~${months} months): force a rich, detailed total-body silhouette change matching this long horizon — max freedom to reshape waist, fat pads, and muscle volume.`
      : months <= 3
        ? `TIMELINE MULTIPLIER (~${months} months): keep change gradual — slightly tighter, subtle fat reduction, toning — still visibly different.`
        : `TIMELINE MULTIPLIER (~${months} months): scale visible progress to this mid-length horizon.`;
...
function buildModelInput(
  ...
  if (name.includes("flux-kontext") || /^black-forest-labs\//i.test(name)) {
    ...
    // Upsampling: ON for long timelines (1y+) or large BF jumps.
    // OFF only for short/mid shirtless-risk strong fat-loss (E005 trade-off).
    const highShirtlessRisk =
      extras.fat === "decrease" && intensity === "strong" && !longHorizon;
    const useUpsampling =
      longHorizon || (needStrongEdit && !highShirtlessRisk);
...
async function runPrediction({
  ...
  if (prediction.status !== "succeeded" && prediction?.urls?.get) {
    ...
    prediction = await pollPrediction(prediction.urls.get, token, remaining);
  }
...
async function generateWithReplicate({
  ...
  // Per-attempt ~50s so 2–3 models fit under Vercel 180s.
  ...
  for (let i = 0; i < uniqueAttempts.length; i++) {
    ...
    try {
      ...
      // Max/Pro often E005 on the same shirtless input — jump to Dev instead of burning another ~50s.
