...
function buildForcedChangeBlock({ fat, bfNow, bfGoal, level, zones }) {
  ...
  if (fat === "increase" || goal > now + 1) {
    return [
      "HOWEVER, change the body composition significantly from the source photo:",
      "Make the waist thicker and the midsection softer.",
      `Add visible soft subcutaneous fat so the physique matches about ${goal}% body fat.`,
  ...
  const zoneHint = (zones || []).includes("abs")
    ? "Prioritize reshaping the waist and lower abdomen."
    : "Reshape the full torso, not only one detail.";

  const hard =
    level >= 2
      ? [
          "HOWEVER, the body composition has changed significantly from the original image:",
          "The waist is noticeably narrower and tighter — smaller waist circumference than the source.",
          "Subcutaneous fat over the lower abdomen and chest is visibly reduced.",
          `Match a lean about ${goal}% body fat physique (was about ${now}%).`,
          "The abdominal muscles and obliques are sharply defined and chiseled in a realistic photo way.",
          "Shoulder and arm muscle separation is visibly enhanced.",
          zoneHint,
          "Do NOT keep the same soft belly or the same waist width as the source.",
        ]
      : [
          "HOWEVER, change the body composition clearly from the source:",
          "Narrow the waist and reduce the soft fat layer on the lower abdomen.",
...
function buildPrompt(maal, intensity = "moderate", extras = {}) {
  ...
  return [
    "Edit this photograph. Keep the exact same person, face, hair, room, pose, camera and clothing.",
    ...
    raisingFat
      ...
      : meaningfulLoss
        ? "FORBIDDEN: near-identical copy, same waist width, same soft lower-abdomen fat pad, abs-only paint-over. REQUIRED: narrower waist + visibly less belly fat."
...
function buildModelInput(
  ...
  if (name.includes("flux-kontext") || /^black-forest-labs\//i.test(name)) {
    ...
    const needStrongEdit =
      (extras.fat === "decrease" || extras.fat === "increase") &&
      abs >= 4 &&
      intensity !== "subtle";
