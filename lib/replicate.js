...
/**
 * Timeline multiplies change force: 3mo dampens, 6mo neutral, 12mo+ dramatic.
 * Mirrors changeForce ladder (1 → 2 → 4) as additive level boost (−1 … +2).
 */
function timelineScaleBoost(months) {
  const m = Number(months) || 3;
  if (m <= 4) return -1; // ~3mo (changeForce 1)
  if (m <= 8) return 0; // ~6mo (changeForce 2)
  return 2; // ~12mo+ (changeForce 4)
}
...
const HORIZON_COPY = {
  ...
  // changeForce 1 — baseline visible (~3 months)
  "12w": {
  ...
  // changeForce 2 — ~2× the 3mo change (~6 months)
  "24w": {
  ...
  // changeForce 4 — top UI rung (~12 months); former 18m drama maps here
  "12m": {
    label: "about 12 months from now",
    months: 12,
    change:
      "A full athletic recomposition — roughly twice the visible change of a 6-month look: dramatically narrower waist, major soft-tissue loss across midsection and flanks, clear natural muscle separation and athletic outline — anatomically correct athletic build, natural and balanced body proportions, muscle growth is strictly proportionate to the original skeletal structure, no exaggerated or unnatural swelling of the arms — still a real photograph of this person, not a caricature",
  },
  "52w": {
    label: "about 12 months from now",
    months: 12,
...
function buildPromptHolistic(maal, intensity = "moderate", extras = {}) {
  ...
  const timelineForce =
    months >= 12
      ? `TIMELINE MULTIPLIER (~${months} months, distance ${distance?.score ?? "?"}/10): rich total-body silhouette change matching this long horizon — reshape waist, soft-tissue pads, and muscle volume with clear visual delta. anatomically correct athletic build, natural and balanced body proportions, muscle growth is strictly proportionate to the original skeletal structure, no exaggerated or unnatural swelling of the arms.`
      : months > 4 && months <= 6
        ? `TIMELINE MULTIPLIER (~${months} months, distance ${distance?.score ?? "?"}/10): A significant and highly noticeable body transformation, clearly sculpted midsection, distinct muscle separation across the chest and arms.`
        : months > 2 && months <= 4
          ? `TIMELINE MULTIPLIER (~${months} months, distance ${distance?.score ?? "?"}/10): A clearly visible reduction in surface body fat, noticeably tighter waistline, early but distinct muscle definition.`
          : months <= 3
            ? `TIMELINE MULTIPLIER (~${months} months, distance ${distance?.score ?? "?"}/10): clear recomposition — narrower waist, reduced midsection soft tissue, early muscle outline — still visibly different from the source.`
            : months > 6
              ? `TIMELINE MULTIPLIER (~${months} months, distance ${distance?.score ?? "?"}/10): clear strong mid-horizon progress — stronger than early months, scale visible change to this timeframe. anatomically correct athletic build, natural and balanced body proportions, muscle growth is strictly proportionate to the original skeletal structure, no exaggerated or unnatural swelling of the arms.`
              : `TIMELINE MULTIPLIER (~${months} months, distance ${distance?.score ?? "?"}/10): clear strong mid-horizon progress — stronger than early months, scale visible change to this timeframe.`;
...
async function runPrediction({
  ...
  // Log exact prompt Flux/SDXL receives (slim prompts are short; truncate only if huge).
...
async function generateWithReplicate({
  ...
  // Cost-aware order: Max for hard transforms, Pro for mild — then Dev (+ strong Dev retry).
  // SDXL only for non-body edge cases; never returned as success for Future You body edits.
