...
/** Exact Frøydis calibration — ~3 months. */
...
/** Exact Frøydis calibration — ~6 months. */
 ...
 * @returns {string[]}
 */
function fatLossDetails(phase, horizonLabel, { bfGoal, compact = false, months } = {}) {
  ...
  if (phase === "early") {
    // Exact ~3mo calibration (no slight/subtle/gradual).
    return [
      `FØR/ETTER (fat loss, early phase ${horizonLabel}): ${TIMELINE_CALIBRATION_3MO}`,
    ];
  }
  if (phase === "mid") {
    // Exact ~6mo calibration when near 6 months; otherwise strong mid language.
    const nearSix = Number.isFinite(m) ? m > 4 && m <= 6 : true;
    if (nearSix) {
      return [
    ...
    return [
      `FØR/ETTER (fat loss, mid phase ${horizonLabel}): flatter midsection, clearer waist taper, reduced soft tissue at the flanks and lower belly; distinct muscle outlines under thinner cover${bfHint} — not a near-identical silhouette.`,
    ];
  }
  if (compact) {
    return [
