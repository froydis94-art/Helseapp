...
/**
 * Front-loaded progress (see lib/transformProgress.js): 3mo≈0.53, 6mo≈0.78, 12mo≈0.95.
 * NOT a 1×→2×→4× visual-force ladder — diminishing returns after early change.
 */
/** ~3 months — demand obvious pixel change (Flux under-edits timid language). */
...
/** ~6 months — further toward goal; increment smaller than 0→3 (diminishing returns). */
...
/**
 * ~12 months — near-goal refined completion; small increment from 6→12.
 ...
 * @returns {string[]}
 */
function fatLossDetails(phase, horizonLabel, { bfGoal, bfNow, months, compact = false } = {}) {
  let bfHint = "";
  try {
    const { bfAtHorizon } = require("./transformProgress");
    const interim = bfAtHorizon(bfNow, bfGoal, months);
    if (interim != null) {
      bfHint = ` matching about ${interim}% body fat at this horizon`;
    } else {
      const goal = Number(bfGoal);
      if (Number.isFinite(goal) && goal > 0) bfHint = ` matching about ${goal}% body fat`;
    }
  } catch (_) {
    const goal = Number(bfGoal);
    if (Number.isFinite(goal) && goal > 0) bfHint = ` matching about ${goal}% body fat`;
  ...
  if (phase === "early") {
    // ~3mo front-loaded (~53% of journey) — force obvious change.
    return [
      `FØR/ETTER (fat loss, early phase ${horizonLabel}): ${TIMELINE_CALIBRATION_3MO}${bfHint}`,
    ];
  }
  if (phase === "mid") {
    // ~6mo (~78% of journey).
  ...
  // Long / extended (≥12mo, ~95%) — former 18m drama lives here.
  if (compact) {
    return [
